...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(item => {
    let splitRecipe = item.split(' ');
    let joinRecipie = splitRecipe.slice(2).join(' ')
    result.push(joinRecipie);
  })
  return result;
};
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(item => {
    let actionVerb = item.split(' ')
    result.push(actionVerb[0]);
  })
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i] % 2 === 0 ) {
      //in this array, splice at i 1 position
      arr.splice(i,1);
      i--;
    }
  }
